...
if not st.session_state.autenticato:
    # CSS avanzato per centrare il pannello, stilizzare i campi e rimuovere i bordi di default
    st.markdown("""
    <style>
    ...
    # Conversione del logo in Base64 per la massima stabilità di caricamento
    ...
    # Rendering del blocco grafico centrale
    if img_data_base64:
        st.markdown(f"""
    ...
    # Modulo di inserimento credenziali bilanciato in colonne centrali
    ...
    with col_l2:
        ...
        # Pulsante di accesso primario stilizzato ad alta visibilità
    ...
    st.stop() # Blocca l'esecuzione delle connessioni finché non si è loggati
...
# --- AREA AMMINISTRATIVA / DASHBOARD ---
    ...
    # --- CONFIGURAZIONE DELLA SIDEBAR CRIPTTATA IN BASE64 ---
    with st.sidebar:
    ...
    with scelta_tab[4]:
        ...
        with col_tabella:
            ...
            else: st.info("Nessuna assunzione registrata.")
            ...
            with st.form("form_nuovo_cliente", clear_on_submit=True):
                ...
                if submit_cliente:
                    if ragione_sociale and partita_iva:
                        ...
                        st.rerun()
                    else: st.error("❌ Ragione Sociale e Partita IVA sono obbligatori.")
        ...
        with col_elenco:
            ...
            if st.session_state.lista_clienti:
                ...
                if not df_modificato.equals(df_clienti):
                    ...
                    st.rerun()
            else: st.info("Nessun cliente in archivio.")

    # --- TAB 8: DATABASE ANAGRAFICO CANDIDATI ---
    with scelta_tab[7]:
        st.subheader("👥 Database Anagrafico Globale Candidati")
        col_ord1, col_ord2 = st.columns([2, 2])
        with col_ord1: ordine_scelto = st.selectbox("Ordina l'elenco per:", ["Ultimi Arrivi", "Ordine Alfabetico (A-Z)"])
        with col_ord2:
        ...
        if not tutti: st.info("Nessun candidato registrato nel database cloud.")
        else:
            ...
            elif ordine_scelto == "Ordine Alfabetico (A-Z)": tutti = sorted(tutti, key=lambda x: x.get('nome', '').lower())
                ...
                with st.expander(f"🔍 Apri Scheda Completa: {c['nome']}"):
                    ...
                    * **Orientamento IA:** {c.get('orientamento', 'Profilo acquisito.')}
                    ...
                    with col_pop2:
                        ...
                        if url_pdf and url_pdf.startswith("http"): st.link_button("📥 Scarica PDF Originale", url_pdf, use_container_width=True)
                        else: st.caption("Nessun PDF originale allegato.")
                    with col_pop3:
                        if st.button("🗑️ Elimina Risorsa", key=f"pop_del_{c['id']}", use_container_width=True, type="secondary"):
                            if url_pdf and "curriculum/" in url_pdf:
                                try: supabase.storage.from_("curriculum").remove([url_pdf.split("curriculum/")[-1]])
                                catch: pass
