...
# --- AREA AMMINISTRATIVA / BACKOFFICE ---
    ...
    if not st.session_state.autenticato:
        ...
        with scelta_tab[8]:
            ...
            st.caption("Layout ottimizzato: le card sono bloccate a un'altezza di 382px.")

            # 1. Rilettura annunci real-time
            ...
            if not annunci_flag_vetrina:
                ...
                for index, a in enumerate(annunci_flag_vetrina):
                    ...
                    with cols[index]:
                        st.markdown(f'''<a href="/?job={a['id']}" style="display: block; width: 100%; aspect-ratio: 395/704; background-image: url(\'{img_v_url}\'); background-size: cover; background-position: center; border-radius: 8px; border: 1px solid #E2E8F0;"></a>''', unsafe_allow_html=True)
            ...
            with col_search1: search_ruolo = st.selectbox("🔍 Cosa stai cercando?", ["Tutti i Ruoli"] + ruoli_disponibili)
            with col_search2: search_citta = st.selectbox("📍 Dove?", ["Tutte le Sedi"] + citta_disponibili)
            ...
            # Utilizziamo un ciclo per creare le card a coppie
