...
# 2. CSS Custom Premium Integrato per l'interfaccia aziendale e portale pubblico
st.markdown("""
    <style>
...
# --- DATABASE CONDIVISO PERSISTENTE TRA SCHEDE (ANTI-AZZERAMENTO) ---
@st.cache_resource
...
# Inizializzazioni statiche menu
...
if "job" in query_params:
    ...
    if annuncio_selezionato:
        ...
        st.markdown(f"<span class='meta-badge'>📍 Sede: {annuncio_selezionato['Sede']}</span><span class='meta-badge'>💸 Inquadramento: {annuncio_selezionato['Importo']} € ({annuncio_selezionato['Inquadramento']})</span>", unsafe_allow_html=True)
...
# --- SUITE AMMINISTRATIVA INTERNA PRIVATA ---
    ...
    if not st.session_state.autenticato:
        ...
        # Barra laterale di monitoraggio
        with st.sidebar:
            ...
            if not stanze_attive: st.caption("Nessun colloquio attivo ora.")
                ...
                for s in stanze_attive:
                    st.markdown(f'<div style="background-color:#F0FDF4; padding:10px; border-radius:8px; margin-bottom:8px;"><span style="font-size:11px; font-weight:bold; color:#166534;">📞 IN CORSO ({s["Operatore_Call"]})</span><br><span style="font-size:12px;">{s["Nome"]}</span><br><a href="{s["Meet_Link"]}" target="_blank" style="font-size:12px;font-weight:bold;color:#1a73e8;text-decoration:none;">🔗 Connettiti</a></div>', unsafe_allow_html=True)
            st.markdown("<br>", unsafe_allow_html=True)
        ...
        # Barra di Navigazione centralizzata a 7 Bottoni
        c_nav = st.columns(7)
        ...
        if st.session_state.current_menu == "📢 Annunci":
            ...
            with col_sx:
                ...
                uploaded_file = st.file_uploader("🖼️ Carica Copertina Annuncio", type=["png", "jpg", "jpeg"])
            ...
            with col_centro:
                ...
                st.markdown('<div class="saas-box">', unsafe_allow_html=True)
                note_job = st.text_area("✍️ Requisiti inseriti manualmente", placeholder="Dettagli...", height=150)
                if st.button("🪄 Ottimizza e Correggi con IA", use_container_width=True):
                    if note_job: st.session_state.ai_text_output = f"La DEI REALI Srl seleziona profili qualificati per la posizione di {titolo_job if titolo_job else 'Operatore'} presso le sedi aziendali di {indirizzo_job if indirizzo_job else 'Palestrina'}. Si richiede seria attitudine professionale per turni lavorativi strutturati. Note aggiuntive: {note_job}."
                    else: st.warning("Inserisci del testo per attivare l'IA!")
                if st.session_state.ai_text_output:
                    st.markdown(f'<div class="ai-box"><b>🧠 Testo Ottimizzato IA:</b><br>{st.session_state.ai_text_output}</div>', unsafe_allow_html=True)
                ...
                if st.button("🚀 PUBBLICA E GENERA LINK WEB", use_container_width=True):
                    if titolo_job:
                        ...
                        st.success("🎉 Pubblicato sul portale esterno!"); st.session_state.ai_text_output = ""; st.rerun()
                    else: st.error("Inserisci il titolo posizione.")
                st.markdown('</div>', unsafe_allow_html=True)
            with col_dx:
                st.markdown("### 📋 Annunci Attivi & Condivisione")
                for ann in db_globale["annunci"]:
                    ...
                    st.markdown(f"<b>📢 {ann['Posizione']}</b><br><span style='font-size:12px;color:#475569;'>📍 {ann['Sede']} | 💸 {ann['Importo']}€</span>", unsafe_allow_html=True)
        ...
        # ==========================================
            ...
            with col_ass1:
                st.markdown("#### 📋 Elenco Risorse in Fase di Onboarding")
                for ass in db_globale["assunzioni"]:
                    ...
                    """, unsafe_allow_html=True)
            with col_ass2:
                st.markdown("#### ➕ Registra Nuova Pratica")
        ...
        # ==========================================
            ...
            with kpi1: st.metric("Annunci Online", len(db_globale["annunci"]))
            with kpi2: st.metric("Candidature Ricevute", len(db_globale["candidati"]))
        ...
        # ==========================================
            ...
            with col_cli2:
                ...
                with st.form("add_cli_form"):
                    az_n = st.text_input("Ragione Sociale Azienda")
                    az_s = st.text_input("Settore di riferimento")
                    az_r = st.selectbox("Account Manager Interno Assegnato", ["Danilo", "Dionisio"])
